...
VarKAccPlot <- {qplot(ks,kAccuracy,xlab = "k",ylab = "Accuracy in %",geom = "line",main = "Accuracy according to varying k for all persons seperated")+
    geom_errorbar(aes(x=ks, ymin=kAccuracy-kAccStd, ymax=kAccuracy+kAccStd), width=0.5, alpha= I(1/2))
}
ggsave("Final/preIndiAccVarK.png")

{
  varKAccPlottimePlot <-  qplot(ks,kTime,xlab = "k",ylab = "Time in Seconds",main = "Time according to varying k for all persons seperated")+
    geom_errorbar(aes(x=ks, ymin=kTime-kTimeStd, ymax=kTime+kTimeStd), width=0.5, alpha= I(1/2))
}
ggsave("Final/PreIndiTimeVarK.png")

...
kTimeStd = c()
for (k in ks){
  AllShuffle(SingleData,0.7,single = TRUE)
  acc = c()
  time = c()
  for (i in 1:2){
    start = Sys.time()
...
VarKAccPlot <- {qplot(ks,kAccuracy,xlab = "k",ylab = "Accuracy in %",geom = "line",main = "Accuracy according to varying k for a single person")+
    geom_errorbar(aes(x=ks, ymin=kAccuracy-kAccStd, ymax=kAccuracy+kAccStd), width=0.5, alpha= I(1/2))
}
ggsave("Final/preSingleAccVarK.png")

{
  varKAccPlottimePlot <-  qplot(ks,kTime,xlab = "k",ylab = "Time in Seconds",main = "Time according to varying k for a single person")+
    geom_errorbar(aes(x=ks, ymin=kTime-kTimeStd, ymax=kTime+kTimeStd), width=0.5, alpha= I(1/2))
}
ggsave("Final/PreSingleTimeVarK.png")

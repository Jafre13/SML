...
p1 = qplot(seq_along(accuracies),1,accuracies) +ylab("Accuracy in %")+geom_smooth(method="glm",formula = y ~ splines::bs(x,3), se=FALSE)

p2 = qplot(seq_along(times),times) +scale_x_continuous(name = "cluster size",limits=k)+ylab("Seconds")+geom_smooth(method="glm",formula = y ~ splines::bs(x,3), se=FALSE)
p1
